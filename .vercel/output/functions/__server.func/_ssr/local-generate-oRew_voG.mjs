import { n as stripQcFromStem, r as thickenStem, t as resolveExplain } from "./explain-D5oGm6vd.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/local-generate-oRew_voG.js
var QC = [
	{
		key: "relevance",
...
var GENERIC_DISTRACTORS = QC.map((q) => q.label);
function qcByKey(key) {
	return QC.find((q) => q.key === key) ?? QC[0];
}
function pickRandomQc(exclude) {
	const pool = exclude ? QC.filter((q) => q.key !== exclude) : QC;
	return pool[Math.floor(Math.random() * pool.length)] ?? QC[0];
}
function fourDistractors(primaryLabel, rng = Math.random, excludeKey) {
	const skip = /* @__PURE__ */ new Set([primaryLabel]);
	if (excludeKey) {
...
function trap(rng, motiveA, motiveB, herring, email) {
	const d = amt(rng, 3, 27);
	return stripQcFromStem(` Tờ trình của giám đốc tài chính gửi ủy ban kiểm toán lập luận rằng cách xử lý hiện tại phản ánh đúng nhu cầu người sử dụng vì ${motiveA}, và rằng ${motiveB} cũng ủng hộ cách ghi nhận/trình bày này. Chi tiết nhiễu: ${herring} Ban quản trị trích dẫn việc đó như “tiền lệ” hoặc “bằng chứng chính sách đã minh bạch.” Email nội bộ ngày ${String(d).padStart(2, "0")}/12: “${email}”`);
}
function buildFor(qc, rng) {
	...
	switch (qc.key) {
		case "relevance": {
			if (alt) {
				...
				const patent = pick(rng, [
					"hoạt chất độc quyền",
					"bản quyền phần mềm lõi",
					"nhãn hiệu xuất khẩu"
				]);
				return {
...
function generateLocalIssue(opts) {
	...
	const y1 = built.scenario.match(/20X[0-9]/)?.[0] ?? "20X6";
	const other = pick(rng, COUNTERPARTIES);
...
//#endregion
export { qcByKey as a, pickRandomQc as i, fourDistractors as n, generateLocalIssue as r, QC as t };
